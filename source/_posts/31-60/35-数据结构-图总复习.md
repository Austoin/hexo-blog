...
每一步入度为 0 的顶点若始终唯一，则拓扑序唯一。

### 例题 13：关键路径核心量

1. `ve`：事件最早发生时间（前推取 max）
2. `vl`：事件最晚发生时间（回推取 min）

### 例题 14：关键活动判定

活动 `ai` 满足 `e(i) == l(i)` 时为关键活动。

### 例题 15：关键路径本质

AOE 网关键路径是“决定总工期的最长路径”。
